...
from src.utils import HParams, preprocess, iter_data
...
class Model(object):
    def __init__(self, nbatch=128, nsteps=64):
        ...
        global params
        params = [np.load("src/model/%d.npy" % i) for i in range(15)]
        ...
        def transform(xs):
            ...
            print(
                "%0.3f seconds to transform %d examples"
                % (time.time() - tstart, n)
            )
            return features
